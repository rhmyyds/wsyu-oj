  ...
  <div class="add-exam-component-box">
    <div class="add-exam-component">
      ...
      <div class="exam-select-question-box">
        ...
        <!-- 显示竞赛已经存在的题目 -->
        <el-table height="425px" :data="formExam.examQuestionList" class="question-select-list">
...
<script setup>
import { examAddService, addExamQuestionService, getExamDetailService, editExamService, delExamQuestionService, publishExamService} from "@/api/exam"
...
const type = useRoute().query.type  // 判断是添加还是编辑的变量
...
// 添加和修改竞赛基础信息，然后获取竞赛id
async function saveBaseInfo() {
  const fd = new FormData()   // 组装数据
  for (let key in formExam) {
    if (key === 'examDate') {
      fd.append('startTime', formExam.examDate[0]);
      fd.append('endTime', formExam.examDate[1]);
    } else if(key !== 'startTime' && key !== 'endTime'){  // 因为获取竞赛详情后端会把这两个参数传过了但是前端是封装在examDate里面，所以不能在存一遍了
...
const questionList = ref([]) // 添加题目题目列表题目展示的数据
const total = ref(0)
async function getQuestionList() {
  // 把 存在的数据放进  请求参数中 防止添加的数据添加到展示的题目列表中
  // 从表中 formExam.examQuestionList 获取数据然后传给排除用的字符串
  const str = formExam.examQuestionList && formExam.examQuestionList.length ? formExam.examQuestionList.map(item => item.questionId).join(';') : '';
  console.log("excludeIdStr",str);
  params.excludeIdStr = str;
  const result = await getQuestionListService(params)
  console.log(result)
  questionList.value = result.rows  // 赋值添加题目题目列表题目展示的数据
...
const questionIdSet = ref([])  // 方框选中的题目id
...
async function deleteExamQuestion(examId, questionId) {
  ElMessageBox.confirm(
    ...
    .then(async () => {
        ElMessage({ type: 'success', message: '竞赛题目删除成功',});
...
async function getExamDetailById(examId) {
  ...
  // formExam.examQuestionList = []  // 清空之前查看的信息
  Object.assign(formExam, examDetail.data)  // 后面的赋值到前面  Object.assign() 方法不仅会覆盖目标对象中与源对象同名的属性，还会将源对象中目标对象不存在的属性添加到目标对象中，从而实现对象属性的合并和扩展。
