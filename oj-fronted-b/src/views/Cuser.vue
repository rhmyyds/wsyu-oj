<template>
  ...
  <!-- 表格 -->
  <el-table height="700px" :data="userList">
    ...
    <el-table-column prop="status" width="90px" label="用户状态"> <!--  status传给组件，然后在使用默认插槽获取组件实例然后获取组件的值 -->
      <template #default="{ row }">
        <el-tag type="success" v-if="row.status">正常</el-tag>
        <el-tag type="error" v-else>拉黑ing</el-tag>
      </template>
    </el-table-column>
    <el-table-column label="操作" width="80px" fixed="right">
      <template #default="{ row }">
        <el-button class="red" v-if="row.status === 1" type="text" plain @click="onUpdateUserStatus(row.userId, 0)">拉黑</el-button>
...
import { getUserListService,updateStatusService } from '@/api/cuser'
...
const userList = ref([])  // 页面展示的数据
const total = ref(0)   // 总数据个数

async function getUserList() {
  const ref = await getUserListService(params)  // 请求之后把值存入页面的展示数据中
