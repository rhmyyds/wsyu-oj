...
CREATE TABLE `group_capacity` (
    ...
    `group_id` varchar(128) NOT NULL DEFAULT '' COMMENT 'Group ID，空字符表⽰整个集群',
    `quota` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '配额，0表⽰使⽤默认值',
    `usage` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '使⽤量',
    `max_size` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '单个配置⼤⼩上限，单位为字节，0表⽰使⽤默认值',
    ...
    `max_history_count` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '最⼤变更历史数量',
    `gmt_create` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `gmt_modified` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`),
