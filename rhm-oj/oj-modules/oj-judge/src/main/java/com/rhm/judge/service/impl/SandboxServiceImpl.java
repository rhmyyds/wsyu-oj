...
import com.rhm.judge.callback.DockerStartResultCallback;
import com.rhm.judge.callback.StatisticsCallback;
import com.rhm.judge.domain.CompileResult;
...
public class SandboxServiceImpl implements ISandboxService {
    ...
    private CompileResult compileCodeByDocker() {
        String cmdId = createExecCmd(JudgeConstants.DOCKER_JAVAC_CMD, null, containerId);  // 编译代码的指令和相关参数
    ...
    private SandBoxExecuteResult executeJavaCodeByDocker(List<String> inputList) {
        ...
        for (String inputArgs : inputList) {
            ...
            StatisticsCallback statisticsCallback = statsCmd.exec(new StatisticsCallback()); // 计算内存占用
            stopWatch.start();
            DockerStartResultCallback resultCallback = new DockerStartResultCallback();
            try {
                dockerClient.execStartCmd(cmdId)
